"""Keep the original 26-second soundtrack and cuts; replace hero portraits with archived UGC."""
from pathlib import Path
import subprocess,re,uuid
ROOT=Path(__file__).resolve().parents[2]
PROD=ROOT/'productions/original-ugc'
...
def prepare():
 ...
 dest=ROOT/'output/original-ugc';dest.mkdir(parents=True,exist_ok=True)
 return dest
...
def main():
 ...
 for brand,width,bottom in [('guanyi',170,30),('yeadon',145,25)]:
  run('ffmpeg','-v','error','-y','-i',dest/'base.mp4','-i',ROOT/f'productions/hd-fast/assets/branding/{brand}.png','-filter_complex',f'[1:v]scale={width}:-1[logo];[0:v][logo]overlay=W-w-35:H-h-{bottom}[v]','-map','[v]','-map','0:a?','-c:v','libx264','-crf','16','-preset','fast','-pix_fmt','yuv420p','-c:a','copy','-movflags','+faststart',dest/f'{brand}-original-ugc.mp4')
 print('Videos:',dest)
